...
function prepareTestConfig() {
    ...
    return {
        ...
        userBundlePath: config_1.default.IAC_BUNDLE_PATH,
...
async function test(testConfig) {
    await setup_1.setup(testConfig);
...
const rules_1 = __webpack_require__(60553);
const policy_engine_1 = __webpack_require__(31372);
async function setup(testConfig) {
    await policy_engine_1.initPolicyEngine(testConfig);
    await rules_1.initRules(testConfig);
}
exports.setup = setup;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.initPolicyEngine = exports.lookupLocalPolicyEngine = exports.InvalidUserPolicyEnginePathError = void 0;
...
async function lookupLocalPolicyEngine({ cachedPolicyEnginePath, userPolicyEnginePath, }) {
    // Lookup in custom path.
    if (userPolicyEnginePath) {
        debugLogger('User configured IaC Policy Engine executable path detected: %s', userPolicyEnginePath);
        if (await file_utils_1.isExe(userPolicyEnginePath)) {
    ...
    // Lookup in cache.
        ...
        if (await file_utils_1.isExe(cachedPolicyEnginePath)) {
            debugLogger('Found cached Policy Engine executable: %s', cachedPolicyEnginePath);
            return cachedPolicyEnginePath;
        }
        else {
            debugLogger('Policy Engine executable was not cached: %s', cachedPolicyEnginePath);
        }
    }
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.RulesBundleLocator = exports.initRules = void 0;
const fs = __webpack_require__(35747);
const tar = __webpack_require__(97998);
const errors_1 = __webpack_require__(55191);
const error_utils_1 = __webpack_require__(36401);
const types_1 = __webpack_require__(94820);
async function initRules(testConfig) {
    const bundleLocator = new RulesBundleLocator(testConfig.cachedBundlePath, testConfig.userBundlePath);
...
class RulesBundleLocator {
    constructor(cachedBundlePath, userBundlePath) {
        this.cachedBundlePath = cachedBundlePath;
...
exports.RulesBundleLocator = RulesBundleLocator;
class InvalidUserRulesBundleError extends errors_1.CustomError {
    constructor(message) {
        super(message);
        this.code = types_1.IaCErrorCodes.InvalidUserRulesBundleError;
        this.strCode = error_utils_1.getErrorStringCode(this.code);
